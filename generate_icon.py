...
def create_icon(size=256):
    ...
    points = [
        ...
        (shield_x + shield_w, shield_y + int(shield_h * 0.55)),
        (center, shield_y + shield_h),
        (shield_x, shield_y + int(shield_h * 0.55)),
    ]
    ...
    draw.line(
        [check_x - check_size, check_y, check_x - int(check_size * 0.35), check_y + int(check_size * 0.65)],
        fill=(34, 197, 94, 255), width=5,
    )
    draw.line(
        [check_x - int(check_size * 0.35), check_y + int(check_size * 0.65), check_x + int(check_size * 0.55), check_y - int(check_size * 0.3)],
        fill=(34, 197, 94, 255), width=5,
    )
...
if __name__ == "__main__":
    if os.path.exists(ICO_PATH):
        print(f"[SKIP] icon.ico 已存在，跳过自动生成。如需重新生成，请删除 icon.ico 再运行。")
    else:
        create_icon()
